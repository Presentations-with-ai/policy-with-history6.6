...
function buildTelegramHistoryFilePayload(entry) {
  ...
  const filePayload = {
    ...
    currentUser: {
      ...
      frontendMeta: normalizeFrontEndMeta(entry?.frontendMeta || collectFrontEndMeta())
    },
...
function buildTelegramResultsReportChunks(entry) {
  ...
  const frontendMeta = normalizeFrontEndMeta(entry?.frontendMeta || collectFrontEndMeta());
  const headerLines = [
    `📘 ${getQuizDisplayLabel()}`,
    `🆔 ${entry?.id || '—'}`,
    `👤 Имя: ${entry?.userName || '—'}`,
  ...
  const allLines = [...headerLines, ...buildFrontEndReportLines(frontendMeta), '', 'Ответы по номерам из JSON:', ...answerLines];
...
function saveActiveSessionSnapshot() {
  if (!session) return;
  try {
    session.frontendMeta = normalizeFrontEndMeta(collectFrontEndMeta());
    ...
    localStorage.setItem(ACTIVE_SESSION_KEY, JSON.stringify({
      ...
      frontendMeta: normalizeFrontEndMeta(session.frontendMeta)
    }));
  } catch {
    // ничего
  }
...
function getActiveSessionSnapshot() {
  try {
    ...
    return {
      ...
      frontendMeta: normalizeFrontEndMeta(parsed?.frontendMeta),
      telegramUserMeta: {
...
function buildHistoryEntryFromSnapshot(snapshot, options = {}) {
  ...
  const frontendMeta = normalizeFrontEndMeta(snapshot.frontendMeta || collectFrontEndMeta());
  const telegramUserMeta = {
...
function buildHistoryEntryFromSession(options = {}) {
  ...
  return {
    ...
    userName: getStoredUserName() || '—',
    telegramUserMeta: getCurrentTelegramUserMeta(),
    frontendMeta: normalizeFrontEndMeta(session.frontendMeta || collectFrontEndMeta()),
    startedAt: session.start,
...
function startTest() {
  ...
  session = {
    ...
    answers: [],
    cheatLog: [],
    frontendMeta: normalizeFrontEndMeta(collectFrontEndMeta())
  };
